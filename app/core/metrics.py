...
webhook_delivered_total = Counter(
    "webhook_delivered_total", "Total webhooks successfully delivered", ["event_type"]
)
...
webhook_retry_total = Counter(
    "webhook_retry_total", "Total webhook retries", ["event_type", "attempt_number"]
)

# --- Gauges (can go up/down) ---
queue_depth_gauge = Gauge(
    "webhook_queue_depth", "Current number of webhooks in delivery queue"
)

redis_up_gauge = Gauge(
    "webhook_redis_up", "Status of Redis connection (1 for up, 0 for down)"
)

active_workers_gauge = Gauge(
    "webhook_active_workers", "Number of active delivery workers"
)
